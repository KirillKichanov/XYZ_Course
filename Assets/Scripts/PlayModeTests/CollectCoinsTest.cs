...
namespace Tests
{
    public class CollectCoinsTest : InputTestFixture
    {
        ...
        public IEnumerator CollectSilverCoinsTestWithEnumeratorPasses()
        {
            ...
            var startCoins = GameObject.Find("GameSession").GetComponent<GameSession>().Data.Inventory.Count("Coin");

            player.transform.position = silverCoin.transform.position;

            yield return new WaitForFixedUpdate();

            var collectedCoins = GameObject.Find("GameSession").GetComponent<GameSession>().Data.Inventory.Count("Coin");
